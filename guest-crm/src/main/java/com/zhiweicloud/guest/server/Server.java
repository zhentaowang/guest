...
/**
 * Created by zhengyiyin on 2017/5/8.
 */
@Service
public class Server {
    public static final int SERVER_PORT = 8096;

    private final MyService.Iface passengerServiceImpl;
    private ExecutorService executor = Executors.newSingleThreadExecutor();

    @Autowired
    public Server(MyService.Iface productServiceImpl) {
        this.passengerServiceImpl = productServiceImpl;
    }

    public void startServer() {
        try {
            TProcessor tprocessor = new MyService.Processor<MyService.Iface>(passengerServiceImpl);
            // 非阻塞异步通讯模型（服务器端）
