...
public class Server {
    ...
    private final
    MyService.Iface permissionServiceImpl;
    private ExecutorService executor = Executors.newSingleThreadExecutor();

    @Autowired
    public Server(MyService.Iface permissionServiceImpl) {
        this.permissionServiceImpl = permissionServiceImpl;
    }

    public void startServer() {
        try {
            TProcessor tprocessor = new MyService.Processor<MyService.Iface>(permissionServiceImpl);
            // 非阻塞异步通讯模型（服务器端）
